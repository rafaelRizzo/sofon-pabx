...
function conditionsSummary(rule: RoutingRule, trunkNameById: Map<string, string>): string {
    const parts: string[] = []
    if (rule.conditions.trunkId) {
        parts.push(`tronco ${trunkNameById.get(rule.conditions.trunkId) ?? rule.conditions.trunkId}`)
    }
    if (rule.conditions.weekdays?.length) {
        parts.push(rule.conditions.weekdays.map((d) => WEEKDAY_LABELS[d]).join("/"))
    }
    if (rule.conditions.startTime && rule.conditions.endTime) {
        parts.push(`${rule.conditions.startTime}-${rule.conditions.endTime}`)
    }
    if (rule.conditions.callerIdPattern) parts.push(`callerId~${rule.conditions.callerIdPattern}`)
    return parts.length > 0 ? parts.join(" · ") : "Sem condições"
}

export function RoutingRulesTable({ routingRules, trunks, loading, onEdit, onDelete }: Props) {
