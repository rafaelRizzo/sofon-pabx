...
async function fetchFlowsRequest(companyId: string): Promise<Flow[]> {
...
export function useFlows(companyId?: string) {
    ...
    const { data: flows = [], isLoading: loading } = useQuery({
        ...
        queryFn: () => fetchFlowsRequest(companyId as string),
        enabled: !!companyId,
    })
    ...
    const createMutation = useMutation({
        mutationFn: (form: FlowForm) => api.post("/flows", form),
    })
    ...
    const updateFlow = async (
        ...
        try {
            await updateMutation.mutateAsync({ flowId, form })
    ...
    const deleteMutation = useMutation({
        mutationFn: (flowId: string) => api.delete(`/flows/${flowId}`),
    })
