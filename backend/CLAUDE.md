...
- **`PROCESS_ROLE`** (`src/config/env.ts`) — controla o que o processo sobe, checado em `server.ts`: `web` (só Fastify + Redis, stateless, escalável em N réplicas), `worker` (AGI + AMI events + jobs de cron, sempre **1 instância só** — porta fixa no AGI e listener único de evento, duplicar causa job rodando 2x em paralelo) ou `all` (default — tudo junto, mesmo processo de sempre; usado em dev local e em VPS que não precisam escalar).
