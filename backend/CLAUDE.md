...
- `inboundroute.repository.ts` → tabela `extensions` via Realtime - contexto `from-trunk-routed`, exten `<did>_<companyAsteriskId>`. Priority 1 sempre `Set(ROUTING_TRUNK_ID=<trunkId>)` (esse sim é o trunkId real, gravado literal na criação/update da rota) - variável de canal lida pelo AGI `queue-route` pra casar `RoutingRule.conditions.trunkId`, sobrevive a qualquer `Goto` intermediário (timecondition/holiday/ivr) até a chamada cair numa fila
- **Chave de roteamento entrante é por empresa, não por trunk**: `[from-trunk]` roteia por `Goto(from-trunk-routed,${EXTEN}_${CHANNEL(accountcode)},1)` - `CHANNEL(accountcode)` (= `Company.asteriskId`) é idêntico em qualquer trunk da mesma empresa, ao contrário de `${TRUNKID}` (setvar do endpoint identificado pelo `res_pjsip_endpoint_identifier_ip`), que pode vir do trunk errado quando 2 trunks da mesma empresa compartilham `host`/IP (operadora multi-conta atrás de 1 SBC - `ps_identifies` ambíguo, resolução não-determinística). Por isso 1 DID só pode estar em 1 Inbound Route (bloqueado 409 em `createInboundRoute` se já existir) - 2 trunks com o mesmo DID colidiriam na mesma chave. `CDR(trunk_id)`/`ROUTING_TRUNK_ID`/`GROUP()` continuam usando o trunkId real (gravado literal na Inbound Route, não afetado). Causa raiz completa, tentativas que não funcionaram (`match_header`) e como diagnosticar: `docs/runbooks/inbound-shared-ip-identify.md`
- **Tronco de entrada de empresa diferente da dona do DID (operadora entregando por tronco errado/compartilhado)**: a chave rápida acima (`accountcode` do tronco de ENTRADA) só bate quando a chamada chega pelo tronco da própria empresa dona do DID. Quando não bate, o `i` de `[from-trunk-routed]` (`base-dialplan.repository.ts`) chama o AGI `resolve-did-route` (`handleResolveDidRoute`, `agi-server.ts`) antes de desistir: lookup global por `DID.number` (único globalmente, ver seção DIDs) achando a empresa dona real, independente de por qual tronco a ligação entrou. `DID_ENTRY` (setado em `[from-trunk]` antes do primeiro `Goto`) guarda o número original porque `${EXTEN}` já virou `"i"` nesse ponto. **`CHANNEL(accountcode)` do canal continua sendo o do tronco físico** (nativo do Asterisk, fixado antes de qualquer dialplan) mesmo depois do Goto certo - sem correção, CDR (isolado por `accountcode`, ver seção CDR) e a gravação (`MIXMONITOR_FILENAME` usa `${CHANNEL(accountcode)}` no path) vazariam pra empresa do tronco físico, não a dona do DID. Por isso `handleResolveDidRoute` grava `RESOLVED_ACCOUNTCODE` (variável simples, via AGI) antes do Goto, e `buildInboundEntries` (`inboundroute.repository.ts`) sobrescreve `CHANNEL(accountcode)` de verdade via `Set()` nativo (`IF($["${RESOLVED_ACCOUNTCODE}" != ""]?...)`, no-op no caminho normal) - nunca via escrita de função por AGI (mesmo motivo de `agiExecGoto` não usar `EXEC Goto`, ver comentário lá). Efeito colateral aceito: Call Quality (`ami-events.ts`, resolve por nome do canal) continua atribuindo à empresa do tronco físico (correto pro propósito dela, mede o link físico) - diverge do CDR de propósito nesse cenário raro
...
- Volumes `external: true` (`postgres_sofon_data`, `redis_sofon_data`, `avatars_sofon_data`) e a network `sofon-agi-bridge` (também `external: true`) precisam existir no host antes do primeiro deploy, senão o Dokploy falha com `external volume/network "..." not found`. Automatizado via `setups/ensure-external-resources.sh` (idempotente) configurado como **Pre-deploy Command** do serviço Compose no Dokploy - roda antes de cada deploy e cria só o que faltar. `dokploy-network` fica de fora (gerenciada pelo próprio Dokploy). Setup manual numa VPS nova: garantir que esse comando está configurado no serviço.
...
- DIDs: `UNIQUE(number)` global (não por empresa) - 1 número pertence a no máximo 1 empresa por vez; reatribuir (`companyId` no update) move o DID entre empresas, nunca duplica. Criar/editar/excluir DID é **admin-only** (`requireAdmin`, não permissão granular) - `dids:manage` não existe pra conceder, só `dids:view` (role="user" só vê os DIDs já vinculados à empresa dele, pra escolher na Inbound Route). Motivo: chave de roteamento de entrada é `<did>_<companyAsteriskId>` (ver `inboundroute.repository.ts`) - DID duplicado entre empresas quebraria essa chave
...
- `trunkId` vs `entryTrunkId` (só chamadas `direction=inbound`): `trunkId` é o tronco **configurado** na Inbound Route (fonte de verdade pra `GROUP()`/`ROUTING_TRUNK_ID`/limite de canais); `entryTrunkId` é o tronco que o Asterisk **de fato** identificou pra aquela chamada (`${TRUNKID}`, setvar nativo do endpoint) - só informativo/consistência, nunca usado pra roteamento. Divergem quando a operadora entrega a ligação por um tronco de empresa diferente da dona do DID (ver seção DIDs/`inboundroute.repository.ts`). Ressalva conhecida: quando 2 trunks da MESMA empresa compartilham host/IP, `entryTrunkId` herda a mesma ambiguidade de identificação do Asterisk documentada em `docs/runbooks/inbound-shared-ip-identify.md` - não é bug novo
...
**Dashboard** - `GET /dashboard/overview` - `{ overview: { extensionsOnline, extensionsOffline, callsToday, callsYesterday, callsThisMonth, callsThisYear, callsOutboundToday } }`; `GET /dashboard/infra` - `{ infra: { uptimeSeconds, network, cpu, memory, disk, recordings, logs } }`
- `overview`: `protectedRoute` só (sem permissão granular, é agregado do que o usuário já vê) - escopado por `req.scope.companyIds`. Ramais online/offline via presença ao vivo (`RealtimeService.getExtensionsStatus`); chamadas via `prisma.cdr.count` (ranges hoje/ontem/mês/ano no fuso `TZ`, sem `groupBy`/`date_trunc`); `callsOutboundToday` filtra `direction: 'outbound'` além do range de hoje
- `infra`: `requireAdmin` - infra compartilhada entre todas as empresas (1 VPS só), não é dado de empresa. CPU/memória via `os.loadavg()`/`os.totalmem()`/`os.freemem()` (nativo, sem shell-out); disco via `df -B1` na partição de `DIALPLAN_EXTRA_DIR` (bind mount real do host); tamanho das gravações via `du -sb` em `/var/spool/asterisk/monitor`, **cacheado em memória do processo por 5min** (não Redis/`CacheManager` - métrica aproximada, sem necessidade de consistência entre réplicas `web`); `uptimeSeconds` via `os.uptime()`; `network` (`rxBytesPerSec`/`txBytesPerSec`) via delta de 2 leituras de `/proc/net/dev` com janela de 1s (mesma técnica de `perCoreUsedPct`), somando todas as interfaces exceto `lo` - **Linux-only** (não roda em dev macOS, mesma limitação de `df`/`du` acima)
  - Em produção Dokploy (rede bridge, ver seção "Produção atual" acima), `network` só reflete o tráfego do próprio container do backend (API/Postgres/Redis) - Asterisk roda nativo no host, fora do namespace de rede do container, então SIP/RTP das chamadas **não aparece** nessa métrica. Só refletiria a VPS inteira (incluindo Asterisk) num deploy em `network_mode: host`. Front rotula o card como "Rede (backend)" pra deixar isso explícito.
...
- **`CallRating`** (`/callcenter/ratings`) - `{ companyId, extensionId, number, uniqueid?, scoreAtendimento(1-5)?, scoreServico(1-5)? }`. **1 linha por chamada**, não por pergunta: `POST` interno (chamado 2x pelo AGI, 1x por pergunta) faz upsert em `(companyId, uniqueid)` (`@@unique`) - a 2ª pergunta preenche a coluna que falta na mesma linha da 1ª, em vez de criar uma linha nova (evita 2 registros pra 1 ligação na listagem). Sem `uniqueid` (só possível chamando a rota manualmente) não dá pra casar com a 1ª pergunta, cria linha própria mesmo. `GET` com filtro `companyId`(obrigatório)/`extensionId`/`number`/`score`(1-5, bate em `scoreAtendimento` OU `scoreServico`)/`startDate`/`endDate`/`limit`/`order` - mesmo padrão de filtro do CDR. Populado pela pesquisa de satisfação IVR pós-atendimento (2 perguntas, ver dialplan abaixo), não por webhook externo.
