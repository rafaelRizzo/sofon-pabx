...
- **`docker-compose.yml`** — 3 services: `postgres` (porta `5433:5432` publicada no host), `redis` e `backend` (`network_mode: host`, obrigatório porque o backend precisa falar com o Asterisk nativo e o AMI em `127.0.0.1`, e `network_mode: host` não permite resolver os services `postgres`/`redis` pelo nome — por isso a URL de runtime na env aponta pra `127.0.0.1:5433`/`127.0.0.1:6379`, não `postgres:5432`/`redis:6379`). Monta `/etc/asterisk/dialplan-extra` e `/var/lib/asterisk/sounds` do host como volume (paths criados pelo `install-asterisk.sh`). `backend` só sobe depois de `postgres`/`redis` ficarem `service_healthy` (`depends_on` com `condition`), e tem seu próprio `healthcheck` batendo em `GET /health` via `bun -e fetch(...)` (imagem `oven/bun` não tem `curl`/`wget`) — reflete o check de Redis feito pela própria rota (ver seção API abaixo).
