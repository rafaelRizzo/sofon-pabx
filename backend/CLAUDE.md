...
- Volumes `external: true` (`postgres_sofon_data`, `redis_sofon_data`, `avatars_sofon_data`) precisam existir no host antes do primeiro deploy, senão o Dokploy falha com `external volume "..." not found`. Automatizado via `setups/ensure-volumes.sh` (idempotente) configurado como **Pre-deploy Command** do serviço Compose no Dokploy - roda antes de cada deploy e cria só o que faltar. Setup manual numa VPS nova: garantir que esse comando está configurado no serviço.
