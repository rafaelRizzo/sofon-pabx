...
// Usado só pelo resync explícito de admin (companies.service.ts) - diferente do reloadDialplan()
// fire-and-forget/debounced de cima (usado pelos CRUDs individuais, onde a request não deve esperar
// o reload), aqui o chamador precisa do resultado real: arquivos/Realtime já estão corretos em disco/
// banco nesse ponto, mas sem reload confirmado o Asterisk continua rodando o dialplan antigo em
// memória até o próximo CRUD ou reload manual - o admin precisa saber disso, não receber sucesso falso.
export async function reloadDialplanNow(): Promise<boolean> {
    return runAmiCommand('dialplan reload')
}
