...
const NAMESPACE = 'routing-rules'

export class RoutingRulesCache {
    ...
    static async getRule(id: string) {
        const cached = await cacheManager.get(`${NAMESPACE}:rule`, id)
        logger.info({ event: cached ? 'cache.hit' : 'cache.miss', namespace: NAMESPACE, key: `rule:${id}` })
        return cached
    }

    static async setRule(id: string, data: any, config?: CacheConfig) {
        await cacheManager.set(`${NAMESPACE}:rule`, id, data, config)
        logger.info({ event: 'cache.set', namespace: NAMESPACE, key: `rule:${id}` })
    }

    static async invalidateRule(id: string) {
        await cacheManager.invalidateByKey(`${NAMESPACE}:rule:${id}`)
        logger.info({ event: 'cache.invalidate', namespace: NAMESPACE, key: `rule:${id}` })
    }
}
