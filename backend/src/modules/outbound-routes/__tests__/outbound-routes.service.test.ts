...
describe('Service.createOutboundRoute', () => {
    it('creates route and writes dialplan to extensions table', async () => {
        const route = await Service.createOutboundRoute({
            ...
            trunkIds: [trunkId],
            patterns: [
                { pattern: '_0[1-9][2-8]XXXXXXXX', prefix: '0', prepend: null, position: 1 },
            ],
        })
    ...
    it('fails with unknown trunk', async () => {
    ...
    it('fails with unknown company', async () => {
        await expect(Service.createOutboundRoute({
            name: 'Bad',
...
describe('Service.updateOutboundRoute', () => {
    ...
    it('replaces patterns and rewrites dialplan', async () => {
        await Service.updateOutboundRoute(routeId, {
            patterns: [
                { pattern: '_55ZX9XXXXXXXX', prefix: null, prepend: null, position: 1 },
            ],
        })

        const old = await prisma.extensions.findMany({ where: { context: 'ramais', exten: '_0[1-9][2-8]XXXXXXXX' } })
        expect(old).toHaveLength(0)

        const newDialplan = await prisma.extensions.findMany({
            where: { context: 'ramais', exten: '_55ZX9XXXXXXXX' },
            orderBy: { priority: 'asc' },
        })
        expect(newDialplan.length).toBeGreaterThanOrEqual(2)
        const dial = newDialplan.find(e => e.app === 'Dial')
        expect(dial?.appdata).not.toContain('Set')
        expect(dial?.appdata).toContain('${EXTEN}@')
    })
})

// ─── addPattern / deletePattern ───────────────────────────────────────────────
describe('Service.addPattern / deletePattern', () => {
    it('adds pattern and writes dialplan', async () => {
        ...
        const dialplan = await prisma.extensions.findMany({ where: { context: 'ramais', exten: '_0800XXXXXXX' } })
        expect(dialplan.length).toBeGreaterThanOrEqual(2)

        await Service.deletePattern(routeId, pattern!.id)

        const after = await prisma.extensions.findMany({ where: { context: 'ramais', exten: '_0800XXXXXXX' } })
...
describe('Service.setTrunks', () => {
    it('replaces trunks and rewrites dialplan', async () => {
        ...
        await prisma.trunk.delete({ where: { id: trunk2.id } }).catch(() => {})
    })
})
...
describe('Service.deleteOutboundRoute', () => {
    it('removes route and cleans dialplan', async () => {
        ...
        const dialplan = await prisma.extensions.findMany({ where: { context: 'ramais', exten: '_55ZX9XXXXXXXX' } })
